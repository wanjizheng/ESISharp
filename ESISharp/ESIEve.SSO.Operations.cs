...
namespace ESISharp {
    ...
    public partial class Sso {
        internal void Authorize() {
            ...
            Process.Start(SsoBuilder.ToString());
            ...
            if (GrantType == OAuthGrant.Authorization && SecretKey != null) {
                ...
                while (!string.IsNullOrEmpty(AccessTokenResponse)) {
                    ...
                    AuthToken = new AuthToken(Token.access_token, Token.token_type, Token.refresh_token,
                        Token.expires_in);
        ...
        private Dictionary<string, string> ParseReplyArguments(string RouterReply) {
            ...
            var ReplyArgs = RouterReply.Split(new[] { UrlHeader }, StringSplitOptions.None)
        ...
        internal void RefreshAccessToken() {
            ...
            var Token = JsonConvert.DeserializeObject<AccessToken>(GetAccessTokenTask.Result);
        ...
        private async Task<string> SsoPost(string Grant, string CodeType, string Code) {
            ...
            SsoClient.DefaultRequestHeaders.Authorization =
                new AuthenticationHeaderValue("Basic", Base64Encode(ClientID + ":" + SecretKey));
            var Response = await SsoClient.PostAsync(new Uri("https://login.eveonline.com/oauth/token"),
        ...
        public TokenVerification VerifyToken() {
            while (VerifyCredentials()) {
                ...
                VerificationClient.DefaultRequestHeaders.Accept.Add(
                ...
                if (!RequestedScopes.Contains(Scope.None) &&
                    !RequestedScopes.TrueForAll(x => Result.Scopes.Contains(x)))
        ...
        /// <summary>Verify information of the callback protocol.
        ///     <para>Key will be create if it doesn't exist, or overwritten if there is an error.</para>
        /// </summary>
