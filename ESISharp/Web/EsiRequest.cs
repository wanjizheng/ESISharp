...
namespace ESISharp.Web {
    ...
    public class EsiRequest {
        ...
        private readonly string CredentialErrorMessage = JsonConvert.SerializeObject(new
        ...
        internal EsiRequest(ESIEve EsiObject, string RequestPath, EsiWebMethod Method, params object[] RequestData) {
            ...
            switch (Method) {
                case EsiWebMethod.Get:
                    RequestMethod = GetAsync;
                    break;
                case EsiWebMethod.Post:
                    RequestMethod = PostAsync;
                    break;
                case EsiWebMethod.AuthGet:
                    RequestMethod = AuthGetAsync;
                    break;
                case EsiWebMethod.AuthPost:
                    RequestMethod = AuthPostAsync;
                    break;
                case EsiWebMethod.AuthPut:
                    RequestMethod = AuthPutAsync;
                    break;
                case EsiWebMethod.AuthDelete:
                    RequestMethod = AuthDeleteAsync;
                    break;
            }
        }
        ...
        private async Task<EsiResponse> AuthGetAsync(params object[] RequestData) {
            ...
            while (AuthObject.SSO.VerifyCredentials()) {
                AuthObject.QueryClient.DefaultRequestHeaders.Authorization =
                    new AuthenticationHeaderValue("Bearer", ActiveAccessToken());
        ...
        private async Task<EsiResponse> AuthDeleteAsync(params object[] RequestData) {
            ...
            while (AuthObject.SSO.VerifyCredentials()) {
                ...
                if (RequestData != null && RequestData.Length != 0) {
                    ...
                    Response = await AuthObject.QueryClient.SendAsync(Message, HttpCompletionOption.ResponseHeadersRead)
