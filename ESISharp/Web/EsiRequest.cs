...
namespace ESISharp.Web
{
    public class EsiRequest
    {
        ...
        internal EsiRequest(EsiConnection esiconnection, ApiPath.Path path, WebMethods method, Data data) : this(esiconnection, path, method)
        {
            if (data.Query != null)
            {
            ...
            if (data.BodyKvp != null && data.BodyDynamic != null)
            {
                // TODO: Create Invalid Data Exception
            }
            else if (data.BodyKvp != null || data.BodyDynamic != null)
            {
                if (data.BodyKvp != null)
                {
                    DataBody = JsonConvert.SerializeObject(data.BodyKvp);
                }
                if (data.BodyDynamic != null)
                {
                    DataBody = JsonConvert.SerializeObject(data.BodyDynamic);
                }
            }
        }
